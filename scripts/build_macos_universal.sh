...
usage() {
  cat <<'EOF'
Usage:
  scripts/build_macos_universal.sh [--configuration Release] [--output-dir <dir>] [--version <ver>] [--skip-zip]
                                  [--sign-identity <identity>]
                                  [--notarize --apple-key-id <id> --apple-issuer-id <issuer> --apple-api-key-file <path>]

What it does:
  1. Publishes CBDB Desktop for osx-arm64 and osx-x64
  2. Verifies the publish file lists match
  3. Builds a universal "CBDB Desktop.app" by lipo-merging every Mach-O file
  4. Optionally signs and notarizes the app bundle
  5. Optionally zips the app bundle with ditto

Notes:
  - Runs a standard restore first, then publishes each macOS RID with --no-restore.
  - Uses a normal directory-style app bundle for macOS universal builds; it does not rely on single-file publish.
  - Keeps the inner executable name as "CBDB".
  - Writes outputs under artifacts/local-macos-universal by default.
EOF
}
...
if [[ -z "${OUTPUT_DIR}" ]]; then
  OUTPUT_DIR="${REPO_ROOT}/artifacts/local-macos-universal"
fi
...
ARM_PUBLISH="${OUTPUT_DIR}/osx-arm64/publish"
X64_PUBLISH="${OUTPUT_DIR}/osx-x64/publish"
BUNDLE_ROOT="${OUTPUT_DIR}/${APP_NAME}.app"
CONTENTS_DIR="${BUNDLE_ROOT}/Contents"
MACOS_DIR="${CONTENTS_DIR}/MacOS"
RESOURCES_DIR="${CONTENTS_DIR}/Resources"
ZIP_PATH="${OUTPUT_DIR}/cbdb-${VERSION}-osx-universal.zip"
NOTARY_ZIP_PATH="${OUTPUT_DIR}/cbdb-${VERSION}-osx-universal.notary.zip"

rm -rf "${ARM_PUBLISH}" "${X64_PUBLISH}" "${BUNDLE_ROOT}"
mkdir -p "${ARM_PUBLISH}" "${X64_PUBLISH}" "${MACOS_DIR}" "${RESOURCES_DIR}"

echo "==> Restoring project"
DOTNET_CLI_HOME=/tmp dotnet restore "${PROJECT_PATH}" --configfile "${REPO_ROOT}/NuGet.Config"

echo "==> Publishing osx-arm64 (${CONFIGURATION})"
...
echo "==> Publishing osx-x64 (${CONFIGURATION})"
DOTNET_CLI_HOME=/tmp dotnet publish "${PROJECT_PATH}" \
  -c "${CONFIGURATION}" \
  -r osx-x64 \
  --self-contained true \
...
echo "==> Comparing publish file lists"
typeset -a arm_files
typeset -a x64_files
arm_files=("${(@f)$(cd "${ARM_PUBLISH}" && find . -type f | sort)}")
x64_files=("${(@f)$(cd "${X64_PUBLISH}" && find . -type f | sort)}")

if [[ "${arm_files[*]}" != "${x64_files[*]}" ]]; then
  echo "arm64 and x64 publish outputs differ; refusing to build universal bundle" >&2
  comm -3 <(printf '%s\n' "${arm_files[@]}") <(printf '%s\n' "${x64_files[@]}") >&2
  exit 1
fi

echo "==> Building universal app bundle"
for rel in "${arm_files[@]}"; do
  rel_path="${rel#./}"
  arm_file="${ARM_PUBLISH}/${rel_path}"
  x64_file="${X64_PUBLISH}/${rel_path}"
  out_file="${MACOS_DIR}/${rel_path}"

  mkdir -p "$(dirname "${out_file}")"

  arm_desc="$(file -b "${arm_file}")"
  x64_desc="$(file -b "${x64_file}")"

  if [[ "${arm_desc}" == *"Mach-O"* || "${x64_desc}" == *"Mach-O"* ]]; then
...
if [[ -n "${SIGN_IDENTITY}" ]]; then
  ...
  echo "==> Signing Frameworks dylibs"
  if [[ -d "${CONTENTS_DIR}/Frameworks" ]]; then
    find "${CONTENTS_DIR}/Frameworks" -type f -name "*.dylib" -print0 | while IFS= read -r -d '' file_path; do
